...
class AdminHomePanel extends React.Component {
      ...
      if (user) {
        ...
        fire.database().ref(`Faculties/`).on('value', (snapshot) => {
          ...
          for (var key in facultyData) {
            AdminData.push(facultyData[key]);
            AdminKeys.push(key);
          }
          this.setState({
  ...
  listFacultyForm(modal4) {
    ...
    const { facultyKeys , Faculties} = this.state;
        ...
        <ModalBody className="peach-gradient" >
          <ul >
            ...
            {Faculties.map((faculty, index) => {
              ...
              return (
                <div>
                  <li className="listItem" key={index} index={index} >{faculty.Faculty} <Button className="blue-gradient" style={{ width: '30%' }} onClick={() => this.extendFaculty(facultyKeys[index])} ><Fa icon="pencil" /></Button> <Button className="blue-gradient" style={{ width: '10%' }} onClick={() => this.extendFaculty(facultyKeys[index])} ><Fa icon="pencil" /></Button> <Button color="danger" onClick={() => this.deleteFaculty(facultyKeys, index)} style={{ width: '10%' }} ><Fa icon="trash"></Fa></Button> </li>
                  <br />
                </div>
              )
...
const mapStateToProps = (state) => {
 ...
 export default connect(mapStateToProps, mapDispatchToProps)(AdminHomePanel)
