...
this.TitlecreateText=this.TitlecreateText.bind(this);
        ...
        console.log('adminsemseterKey',adminSemesterKey);
         ...
         fire.database().ref(`Faculties/${adminFacultyKey}/YearWise/${adminYearKey}/Semmsters/${adminSemesterKey}/Course/`).on('value', (snapshot) => {
              ...
              entryCourse() {
                ...
                const { Course, adminFacultyKey,adminYearKey,userId,adminSemesterKey ,securityPassword } = this.state;
              ...
              edit(course,CourseKey){

                const { Course , securityPassword } = this.state
                this.setState({
                  
                  Course:course.Course,
                  securityPassword:course.securityPassword,
                  courseText:'Edit Course',
                  editMode:true,
                  CourseKey
                })
                this.toggle(10)
              ...
              UpdateCourseInDb(){
                const { Course, adminFacultyKey,adminYearKey,userId,adminSemesterKey ,securityPassword ,CourseKey } = this.state;
                ...
                console.log(`Faculties/${adminFacultyKey}/YearWise/${adminYearKey}/Semmsters/${adminSemesterKey}/Course/${CourseKey}`);
                ...
                if (Course) {
                  ...
                  fire.database().ref(`Faculties/${adminFacultyKey}/YearWise/${adminYearKey}/Semmsters/${adminSemesterKey}/Course/${CourseKey}`).set({
                    Course,
                    securityPassword
                  }).then(() => {
                    alert("Data is Successfully entered.");
              ...
              listCourseForm(modal10) {
                ...
                const {CourseKey, adminSemesterKey,adminFacultyKey,adminYearKey ,securityPassword} = this.state;
                ...
                <div >
                  {
                    console.log(`${CourseKey}     ${index } `   )
                    
                  }
                <li style={{marginLeft:'2%' }} className="listItem" key={index} index={index} >{course.Course}  <Button className="blue-gradient" style={{ width: '30px' }} onClick={() => this.extendCourse(CourseKey[index],adminFacultyKey,adminYearKey,adminSemesterKey)} ></Button>   <Button className="blue-gradient" style={{ width: '30px' }}   onClick={()=>this.edit(course,CourseKey[index])} ><Fa icon="pencil" /></Button> <Button color="danger" onClick={() => this.deleteCourse(CourseKey, index)} style={{ width: '30px' }} ><Fa icon="trash"></Fa></Button> </li>
...
createCourseForm(modal9){
    ...
    const { Course ,securityPassword , editMode , courseText } = this.state;
    return(
        ...
        <h4 className="title">
          <Fa className="fa secondary-fa-pencil" icon="pencil" />{courseText}</h4> 
...
<Input
            ...
            onChange={(e) => { this.setState({ securityPassword: e.target.value }) }}
            required
            validate error="wrong"
            success="right" />
        ...
        <ModalFooter  >
          ...
          {editMode ? 
          <Button className=" blue-gradient" onClick={this.UpdateCourseInDb} >Update</Button>
          :
...
<Grid container spacing={24}>
   ...
   <CardContent>
   <Button className='adminLogOutButton4 blue-gradient' onClick={()=>{this.toggle(9) ; this.TitlecreateText() }} ><Fa icon="pencil" /> Create  Course  </Button>  
   </CardContent>
   </Card>
